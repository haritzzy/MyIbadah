class NiatSholat {
    constructor() {
      this.apiUrl = `https://islamic-api-zhirrr.vercel.app/api/niatshalat`;
      this.fetchData();
    }
    ...
    renderView() {
      ...
      this.dataApi.forEach((item) => {
        const { name, arabic, latin, terjemahan } = item;
  
        const niatSholatContainer = document.getElementById("daftar-niat-sholat");
        ...
        niatSholatContainer.appendChild(accordionContainer);
        ...
        accordionButton.innerHTML = `<article>${counter}. ${name}</article>`;
    ...
    setupSearch() {
      ...
      const list = document.getElementById("daftar-niat-sholat");
  ...
  const NiatSholatObject = new NiatSholat();
  NiatSholatObject.renderView();
